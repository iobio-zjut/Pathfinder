...
extern int Spicker_All_num;
extern int Spicker_All_file_num;
extern int Spicker_Number;
extern core::scoring::ScoreFunctionOP SCORE3;
extern ofstream processdata_1;
extern ofstream processdata_2;
extern ofstream SPICKER_All_Data;
extern std::map<string, string> parametersMap;
extern int N;
extern int Cluster_number;
extern string Cluster_score;
extern bool Save_pdb;
///......................................................................
	...
	namespace abinitio
	{
		...
		void ClassicAbinitio::apply(pose::Pose &pose)
		{
			...
			*/
			...
			string sys = "python ./tools/pre_stage2.py -s " + Cluster_score;
			system(sys.c_str());
		...
		void
		ClassicAbinitio::sampling(core::pose::Pose &pose)
		{

			core::pose::Pose init_pose = pose;
			int serial_number = 0;

			while (serial_number < N || Spicker_Number < Cluster_number)
			{
				...
				cout << '\n';
				cout << "-------------serial_number" << serial_number + 1 << "-----------" << std::endl;

				processdata_1.open("./data/e_data", ios::app);

				processdata_2.open("./data/PCA_data", ios::app);
		...
		ClassicAbinitio::Read_parameters()
		{
			...
			cout << "parameter input number: " << parametersMap.size() << endl;
			Rp.close();
		}

		void
		ClassicAbinitio::get_parameters(map<string, string> &parametersMap)
		{

			stringstream ss;

			if (parametersMap.find("Save_pdb") != parametersMap.end())
			{
				ss << parametersMap["Save_pdb"];
				ss >> Save_pdb;
				ss.clear();
			}
			else
			{
				cout << "====================================parameter 'Save_pdb' is not found in parameters!!!" << endl;
				exit(0);
			}
			...
			if (parametersMap.find("Cluster_number") != parametersMap.end())
			{
				ss << parametersMap["Cluster_number"];
				ss >> Cluster_number;
			}
			else
			{
				cout << "====================================parameter 'Cluster_number' is not found in parameters!!!" << endl;
				exit(0);
			}

			if (parametersMap.find("Cluster_score") != parametersMap.end())
				Cluster_score = parametersMap["Cluster_score"];
			else
			{
				cout << "====================================parameter 'Cluster_score' is not found in parameters!!!" << endl;
				exit(0);
			}
		}

		void
		ClassicAbinitio::initial_file()
		{
			ifstream test("./data");
			if (test.good())
			{
				string sys = "rm -r ./data";
				system(sys.c_str());
			}
			string init_file = "./data/1";
			mkdir(init_file);
			SPICKER_All_Data.open("./data/1/spicker.data1");
			processdata_1.open("./data/e_data", ios::app);
			processdata_2.open("./data/PCA_data", ios::app);

			// output native.pdb as combo0.pdb
			ofstream COM("./data/combo0.pdb");
			for (int r = 1; r <= sequence_lenth; ++r)
			{

				numeric::xyzVector<Real> CA_ = native_pose.residue(r).xyz("CA");

				COM << "ATOM"
					<< right << setw(7) << r
					<< right << setw(4) << "CA"
					<< right << setw(5) << native_pose.residue(r).name3()
					<< right << setw(6) << r
					<< right << setw(12) << CA_.x()
					<< right << setw(8) << CA_.y()
					<< right << setw(8) << CA_.z()
		...
		void
		ClassicAbinitio::mkdir(string &file_name)
		{
			string path;
			if (Save_pdb)
		...
		void ClassicAbinitio::Output_SPICKER_All_Data(core::pose::Pose pose, Real energy)
		{
			++Spicker_All_num;
			++Spicker_Number;
		...
		void ClassicAbinitio::SPICKER_Demand_All(core::pose::Pose Pose)
		{
