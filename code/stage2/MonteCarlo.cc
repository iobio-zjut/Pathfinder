...
ofstream processdata_3;
ofstream out;
utility::vector1<int> SPICKER_NUM;
utility::vector1<core::pose::Pose> SEED_POSE;
utility::vector1<utility::vector1<int>> PIT_SUM;
utility::vector1<utility::vector1<int>> HMM_seq;
utility::vector1<Real> seed_score;
int Trajectory;
...
namespace protocols
{
	namespace moves
	{
		...
		)
		{
			...
			//==========================================================================
			Real tempscore = (50 / 1.5) * atan(POWER / 100);
			score -= tempscore;
		...
		)
		{
			...
			if (accept)
			{
				...
				acc_num++;
				processdata_1 << "POWER  " << POWER << std::endl;

				out << " energy:  " << (*SCORE3)(pose) << " RMSD:   "
					<< core::scoring::CA_rmsd(native_pose, pose) << std::endl;
				...
				for (int i = 1; i <= Seed_number; ++i)
				{
					...
					string file_str1;
					stringstream file_name1;
					dmscore = DM_score(pose, PIT_POSE, i);

					// 若存在相似的，存入seed信息，sum对应位置++
					if (dmscore > 0.55)
					{

						if (dmscore > seed_score[i])
						{

							SEED_POSE[i] = pose;
							seed_score[i] = dmscore;
						}

						if (dmscore > max_DMscore)
						{
							flag = i;
							max_DMscore = dmscore;
						}
				...
				// last_pit表示单向性
				if (PIT_BEGIN != flag && flag != Seed_number + 1)
				{

					// 记录前10次转移
					if (hmm_times <= 10)
					{
						HMM_seq[flag][hmm_times] += 1;
						hmm_times++;
					}
					if (hmm_times > 10)
						hmm_flag = false;
					...
					if (flag != last_pit)
					{
						...
						last_pit = PIT_BEGIN;
						PIT_SUM[PIT_BEGIN][flag] += 1;
						PIT_BEGIN = flag;
					}
				}

				// 统计跳出seed的频数
				if (flag == Seed_number + 1 && PIT_BEGIN != Seed_number + 1)
				{
					PIT_SUM[PIT_BEGIN][flag] += 1;
				}
				// 计算power
				Real energy_power = (*score_function_)(LAST_POSE) - (*score_function_)(pose);
				Real score_power = energy_power / std::max(1.0, angle_distance(pose, LAST_POSE));
				Real kscore = (sequence_lenth * 30.0 / (acc_num + 500)) * score_power;
