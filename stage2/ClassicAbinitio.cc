...
extern core::pose::Pose LAST_POSE;
extern int serial_number;
extern int sequence_lenth;
extern int Seed_number;
extern int PIT_BEGIN;
extern int last_pit;
extern Real POWER;
extern int hmm_times;
extern bool hmm_flag;
extern core::scoring::ScoreFunctionOP SCORE3;
extern utility::vector1<Real> seed_score;
extern utility::vector1<utility::vector1<utility::vector1<Real>>> PIT_POSE;
extern ofstream processdata_1;
extern ofstream processdata_2;
extern ofstream processdata_3;
extern ofstream out;
extern int acc_num;
extern int Trajectory;
extern utility::vector1<int> SPICKER_NUM;
extern utility::vector1<core::pose::Pose> SEED_POSE;
extern utility::vector1<utility::vector1<int>> PIT_SUM;
extern utility::vector1<utility::vector1<int>> HMM_seq;
///......................................................................
...
namespace protocols
{
	namespace abinitio
	{
		...
		ClassicAbinitio::ClassicAbinitio(
			...
			core::kinematics::MoveMapCOP movemap) : movemap_(movemap)
		{
			BaseClass::type("ClassicAbinitio");
			get_checkpoints().set_type("ClassicAbinitio");
			using namespace basic::options;
			...
			if (option[OptionKeys::abinitio::log_frags].user())
			{
				if (!option[OptionKeys::abinitio::debug])
					utility_exit_with_message("apply option abinitio::log_frags always together with abinitio::debug!!!");
		...
		/// was being invoked.
		ClassicAbinitio::ClassicAbinitio(ClassicAbinitio const &src) : // utility::pointer::ReferenceCount(),
																	   Parent(src)
		{
		...
		void ClassicAbinitio::apply(pose::Pose &pose)
		{
			...
			bool success(true);
			SCORE3 = score_stage4_;

			total_trials_ = 0;


			core::import_pose::pose_from_file(native_pose, option[in::file::native](), core::import_pose::PDB_file);
			core::util::switch_to_residue_type_set(native_pose, chemical::CENTROID, true);
			pose = native_pose;

			// initial parameter and file
			Read_parameters();
			get_parameters(parametersMap);
			initial_file(pose);
	
			stage1_cycles_ = static_cast<int>(20000);
			stage2_cycles_ = static_cast<int>(20000);
			stage3_cycles_ = static_cast<int>(20000);
			stage4_cycles_ = static_cast<int>(20000);
		...
		ClassicAbinitio::final_output()
		{
			...
			for (int i = 1; i <= Seed_number; ++i)
			{
				...
				pose_name1 << "./output_files/seed_" << i << ".pdb";

				if (seed_score[i] > 0.7)
				{
					SEED_POSE[i].dump_pdb(pose_name1.str());
				}
			}
			string output_file = "output_files";
			output_seed(output_file);
		}

		void
		ClassicAbinitio::initial_file(core::pose::Pose &pose)
		{

			// input native pose
			core::import_pose::pose_from_file(native_pose, option[in::file::native](), core::import_pose::PDB_file);
			core::util::switch_to_residue_type_set(native_pose, chemical::CENTROID, true);
			sequence_lenth = native_pose.size();

			// check files
			ifstream test("./output_files");
			if (test.good())
			{
				string path = "rm -r ./output_files";
				system(path.c_str());
			}

			string path = "mkdir -p ./output_files";
			system(path.c_str());

			out.open("./HMM_data/out");

			///=============================seed_input=============================
			cout << "=================================================================" << std::endl;
			cout << "                         seed_input" << std::endl;
			cout << "=================================================================" << std::endl;
			...
			for (int i = 1; i <= Seed_number; ++i)
			{
				...
				char tempdata[100] = {0};
				utility::vector1<utility::vector1<Real>> temp_xyz;
				utility::vector1<Real> xyz;
				stringstream seed_name;
				string file_str3;
				seed_name << "./data/e_result/combo" << i << ".pdb";

				seed_name >> file_str3;
				ifstream read_radius1;
				read_radius1.open(file_str3);



				// get combo informations
				while (read_radius1.getline(tempdata, 100))
				{
					...
					{
						xyz.push_back(stof(line.substr(30, 8)));
						xyz.push_back(stof(line.substr(38, 8)));
						xyz.push_back(stof(line.substr(46, 8)));

						temp_xyz.push_back(xyz);
						xyz.clear();
					}


				}
				PIT_POSE.push_back(temp_xyz);
				temp_xyz.clear();

				SEED_POSE.push_back(pose);
			}
			for (int i = 0; i < Seed_number; ++i)
			{
				SPICKER_NUM.push_back(0);
				seed_score.push_back(0.6);
			}
			SPICKER_NUM.push_back(0);

			utility::vector1<int> pit_temp1;
			utility::vector1<int> pit_temp2;

			for (int i = 0; i <= Seed_number; ++i)
			{
				pit_temp1.push_back(0);
			}
			for (int i = 0; i < 10; ++i)
			{
				pit_temp2.push_back(0);
			}

			for (int i = 0; i < Seed_number; ++i)
			{
				PIT_SUM.push_back(pit_temp1);
				HMM_seq.push_back(pit_temp2);
			}

			processdata_1.open("./HMM_data/power");
		}

		void
		ClassicAbinitio::HMM_path(core::pose::Pose &pose)
		{
			...
			while (serial_number < Trajectory + 1)
			{
				...
				cout << "-------------serial_number" << serial_number << "-----------" << std::endl;
				hmm_times = 1;
				PIT_BEGIN = Seed_number + 1;
				last_pit = PIT_BEGIN;
				...
				processdata_2.open("./HMM_data/path", ios::app);
				processdata_3.open("./HMM_data/state", ios::app);

				// 输出转移概率矩阵
				processdata_3 << "test_" << serial_number << std::endl;
				for (int i = 1; i <= Seed_number; ++i)
				{
					for (int j = 1; j <= Seed_number + 1; ++j)
					{
						processdata_3 << PIT_SUM[i][j] << '\t';
					}
					processdata_3 << std::endl;
				}

				// 输出初始概率矩阵
				processdata_2 << "test_" << serial_number << std::endl;
				for (int i = 1; i <= Seed_number; ++i)
				{
					for (int j = 1; j <= 10; ++j)
					{
						processdata_2 << setw(5) << HMM_seq[i][j];
					}
					processdata_2 << std::endl;
				}
				...
				cout << "=================================================================" << std::endl;
				cout << "                         seed_output" << std::endl;
				cout << "=================================================================" << std::endl;
				string output_file = "HMM_data";
				output_seed(output_file);
		...
		void
		ClassicAbinitio::output_seed(string &path)
		{
			...
			ss << "./" << path << "/data";
			ss >> f_path;
			processdata_2.open(f_path.c_str(), ios::app);

			if (path == "output_files")
			{
				processdata_2 << "final_output" << std::endl;
			}
			else
			{
				processdata_2 << "test_" << serial_number << std::endl;
			...
			for (int i = 1; i <= Seed_number; ++i)
			{
				processdata_2
					<< " seed_" << i
					<< "  energy  " << setprecision(4) << (*SCORE3)(SEED_POSE[i]) << "\t"
					<< "  DM_score  " << setprecision(4) << seed_score[i]
					<< std::endl;

				stringstream pose_name1;
				pose_name1 << "./HMM_data/seed_" << i << ".pdb";
				SEED_POSE[i].dump_pdb(pose_name1.str());
			}

			processdata_2 << "SPICKER_NUM" << '\t';
			for (int i = 1; i <= Seed_number + 1; ++i)
			{
				processdata_2 << SPICKER_NUM[i] << '\t';
			}
			processdata_2 << "\n----------------------------------------\n"
						  << std::endl;
			processdata_2.close();
		...
		ClassicAbinitio::get_parameters(map<string, int> &parametersMap)
		{
			...
			if (parametersMap.find("Seed_number") != parametersMap.end())
				Seed_number = parametersMap["Seed_number"];
			else
			{
				cout << "====================================parameter 'Seed_number' is not found in parameters!!!" << endl;
				exit(0);
			}

			if (parametersMap.find("Trajectory") != parametersMap.end())
				Trajectory = parametersMap["Trajectory"];
			else
			{
				cout << "====================================parameter 'Trajectory' is not found in parameters!!!" << endl;
				exit(0);
			}
		}

		Real ClassicAbinitio::DM_score(core::pose::Pose &pose, core::pose::Pose &tempPose)
		{
			Real score_sum = 0;
			Real dm_score = 0;
			Real num = 0;
			...
			{
				for (int j = i + 2; j < sequence_lenth; j++)
				{
					numeric::xyzVector<Real> a1 = (pose.residue(i + 1).atom("CA").xyz());
					numeric::xyzVector<Real> a2 = (pose.residue(j + 1).atom("CA").xyz());

					numeric::xyzVector<Real> b1 = (tempPose.residue(i + 1).atom("CA").xyz());
					numeric::xyzVector<Real> b2 = (tempPose.residue(j + 1).atom("CA").xyz());

					d0 = log(0.001 + fabs(i - j));
		...
		Real ClassicAbinitio::DM_score(
			core::pose::Pose &pose,
			utility::vector1<utility::vector1<utility::vector1<Real>>> &pit_pose,
			int number)
		{
			Real score_sum = 0;
			Real dm_score = 0;
			...
			for (int i = 0; i < sequence_lenth; ++i)
			{
				for (int j = i + 2; j < sequence_lenth; ++j)
				{
					...
					x = pit_pose[number][i + 1][1] - pit_pose[number][j + 1][1];
					y = pit_pose[number][i + 1][2] - pit_pose[number][j + 1][2];
					z = pit_pose[number][i + 1][3] - pit_pose[number][j + 1][3];
					pit_distance = sqrt(x * x + y * y + z * z);

					d0 = log(0.01 + fabs(i - j));
					Real di = fabs(a1.distance(a2) - pit_distance);
		...
		ClassicAbinitio::set_movemap(core::kinematics::MoveMapCOP mm)
		{
			...
			if (smooth_move_small_)
				smooth_move_small_->set_movemap(mm);
			if (brute_move_small_)
				brute_move_small_->set_movemap(mm);
			if (brute_move_large_)
				brute_move_large_->set_movemap(mm);
		}
		...
		void ClassicAbinitio::set_mc(moves::MonteCarloOP mc_in)
		{
			mc_ = mc_in;
			if (trial_large_)
				trial_large_->set_mc(mc_);
			if (trial_small_)
				trial_small_->set_mc(mc_);
			if (smooth_trial_small_)
				smooth_trial_small_->set_mc(mc_);
		}
		...
		void ClassicAbinitio::set_default_scores()
		{
			...
			if (option[OptionKeys::abinitio::stage5_patch].user())
			{ // vats
		...
		void ClassicAbinitio::set_score_weight(scoring::ScoreType type, Real setting, StageID stage)
		{
			...
			if (stage == ALL_STAGES)
				...
				tr.Debug << "stage " << (stage <= STAGE_3a ? stage : (stage - 1)) << (stage == STAGE_3b ? "b " : " ");
			tr.Debug << scoring::name_from_score_type(type) << " " << setting << std::endl;
			if (score_stage1_ && (stage == STAGE_1 || stage == ALL_STAGES))
				score_stage1_->set_weight(type, setting);
			if (score_stage2_ && (stage == STAGE_2 || stage == ALL_STAGES))
				score_stage2_->set_weight(type, setting);
			if (score_stage3a_ && (stage == STAGE_3a || stage == ALL_STAGES))
				score_stage3a_->set_weight(type, setting);
			if (score_stage3b_ && (stage == STAGE_3b || stage == ALL_STAGES))
				score_stage3b_->set_weight(type, setting);
			if (score_stage4_ && (stage == STAGE_4 || stage == ALL_STAGES))
				score_stage4_->set_weight(type, setting);
			if (score_stage4rot_ && (stage == STAGE_4 || stage == ALL_STAGES))
				score_stage4rot_->set_weight(type, setting);
			if (score_stage5_ && (stage == STAGE_5 || stage == ALL_STAGES))
				score_stage5_->set_weight(type, setting); // vats
		}
		...
		*/
		bool ClassicAbinitio::do_stage3_cycles(pose::Pose &pose)
		{
			...
			{
				if (lct1 > 1)
					trials = trial_small(); // only with short_insert_region!
				for (int lct2 = 1; lct2 <= nloop2; lct2++, iteration++)
				{
															  ...
															  false /*fullatom */, true /*fold tree */))
					{
						...
						{ // no convergence check -> no WhileMover
		...
		*/
		bool ClassicAbinitio::do_stage4_cycles(pose::Pose &pose)
		{
			...
			for (Size kk = 1; kk <= nloop_stage4; ++kk)
			{
				...
				if (!prepare_loop_in_stage4(pose, kk, nloop_stage4))
					return false;
		...
		bool ClassicAbinitio::do_stage5_cycles(pose::Pose &pose)
		{ // vats
		...
		void ClassicAbinitio::replace_scorefxn(core::pose::Pose &pose, StageID stage, core::Real /*intra_stage_progress */)
		{
			...
			if (score_stage1_ && (stage == STAGE_1))
				current_scorefxn(*score_stage1_);
			if (score_stage2_ && (stage == STAGE_2))
				current_scorefxn(*score_stage2_);
			if (score_stage3a_ && (stage == STAGE_3a))
				current_scorefxn(*score_stage3a_);
			if (score_stage3b_ && (stage == STAGE_3b))
				current_scorefxn(*score_stage3b_);
			if (score_stage4_ && (stage == STAGE_4))
				current_scorefxn(*score_stage4_);
			if (score_stage4rot_ && (stage == STAGE_4rot))
				current_scorefxn(*score_stage4rot_);
			if (score_stage5_ && (stage == STAGE_5))
				current_scorefxn(*score_stage5_); // vats
		...
		bool ClassicAbinitio::prepare_stage5(core::pose::Pose &pose)
		{ // vats
		...
		// run stage1~5
		void ClassicAbinitio::run_stage1(core::pose::Pose &pose)
		{
			using namespace moves;
			using namespace scoring;
			using namespace scoring::constraints;
		...
		void ClassicAbinitio::run_stage2(core::pose::Pose &pose)
		{
			using namespace moves;
			using namespace scoring;
			using namespace scoring::constraints;
		...
		void ClassicAbinitio::run_stage3(core::pose::Pose &pose)
		{
			using namespace moves;
			using namespace scoring;
			using namespace scoring::constraints;
		...
		void ClassicAbinitio::run_stage4(core::pose::Pose &pose)
		{
			using namespace moves;
			using namespace scoring;
			using namespace scoring::constraints;
		...
		void ClassicAbinitio::run_stage5(core::pose::Pose &pose)
		{
			using namespace moves;
			using namespace scoring;
			using namespace scoring::constraints;
